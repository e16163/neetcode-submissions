...
public:
    int numIslands(vector<vector<char>>& grid) {
        ...
        vector<vector<vector<char>>> adjacent(grid.size(), vector<vector<char>>(grid[0].size(), vector<char>(4, '0')));
        ...
        for (int i = 0; i < grid.size(); i++)
        {
            for (int j = 0; j < grid[0].size(); j++)
            {
                if (grid[i][j]=='1' && visited[i][j]==0)
                {
                    ...
                    dfs(grid, visited, adjacent, num, i, j);
                    num++;
                }
            }
        }
    ...
    void dfs(vector<vector<char>>& grid, vector<vector<int>>& visited, vector<vector<vector<char>>> adjacent, int& num, int i, int j)
    {
